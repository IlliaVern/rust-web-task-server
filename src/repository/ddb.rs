...
impl DDBRepository {
    ...
    pub async fn put_task(&self, task: Task) -> Result<(), DDBError> {
        let mut request = self
            ...
            .item("pK", AttributeValue::S(task.user_uuid))
            .item("sK", AttributeValue::S(task.task_uuid))
            .item("task_type", AttributeValue::S(task.task_type))
            .item("state", AttributeValue::S(task.state.to_string()))
            .item(
                "source_file",
                AttributeValue::S(task.source_file),
            );

        if let Some(result_file) = task.result_file {
            request = request.item("result_file", AttributeValue::S(result_file));
        }
